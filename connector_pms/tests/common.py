...
class TestHotelWubook(TestHotel):
    ...
    def _init_mock_hotel(cls):
        ...
        cls.env["wubook"]._patch_method(
            "create_channel_connector_issue", wubook_create_channel_connector_issue
        )
        ...
        cls.env["wubook"]._patch_method("import_restriction_plans", wubook_ommit)
        cls.env["wubook"]._patch_method("fetch_rplan_restrictions", wubook_ommit)
    ...
    def create_wubook_booking(
        self, creator, checkin, partner, rinfo, channel=0, notes=""
    ):
        ...
        date_diff = date_utils.date_diff(checkin_utc_dt, checkout_utc_dt, hours=False)
        ...
        for k_room, v_room in rinfo.items():
            ...
            # Generate Rooms
            for _ in range(0, len(v_room["occupancy"])):
                rooms.append(k_room)
            # Generate Rooms Occupancies
            for val in v_room["occupancy"]:
                # Generate Rooms Occupancies
                rooms_occu.append({"id": k_room, "occupancy": val})
    ...
    def setUpClass(cls):
        ...
        if default_restriction:
            ...
            cls.restriction_1.write({"wpid": "0"})
