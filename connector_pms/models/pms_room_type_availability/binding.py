...
class ChannelPmsRoomTypeAvailability(models.Model):
    _name = "channel.pms.room.type.availability"
    _inherit = "channel.binding"
    _inherits = {"pms.room.type.availability": "odoo_id"}
    _description = "Channel Availability"

    odoo_id = fields.Many2one(
        comodel_name="pms.room.type.availability", required=True, ondelete="cascade"
    )
    ...
    def refresh_availability(
        ...
        if room_type_bind and room_type_bind.external_id:
            ...
            for i in range(0, date_diff):
                ...
                if room_type_avail_id:
                    # CAVEAT: update channel.pms.room.type.availability if
                    # needed
                    vals_avail = {}
                    ...
                    self.env["pms.room.type.availability"].with_context(
                        {"connector_no_export": True}
                    ).create(
                        {
                            "room_type_id": room_type_bind.odoo_id.id,
    ...
    def import_availability(self, backend, dfrom, dto):
        with backend.work_on(self._name) as work:
            importer = work.component(usage="pms.room.type.availability.importer")
    ...
    def push_availability(self, backend):
        with backend.work_on(self._name) as work:
            exporter = work.component(usage="pms.room.type.availability.exporter")
