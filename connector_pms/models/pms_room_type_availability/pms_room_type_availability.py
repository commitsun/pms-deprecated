...
class PmsRoomTypeAvailability(models.Model):
    ...
    def _default_max_avail(self):
        ...
        channel_room_type = (
            self.env["channel.pms.room.type"].search([("odoo_id", "=", room_type_id)])
            or None
        )
    ...
    room_type_id = fields.Many2one(
        "pms.room.type", "Room Type", required=True, tracking=True, ondelete="cascade"
    )
    channel_bind_ids = fields.One2many(
        comodel_name="channel.pms.room.type.availability",
        inverse_name="odoo_id",
        string="Pms Room Type Availability Connector Bindings",
    )
    ...
    def onchange_room_type_id(self):
        channel_room_type = (
            self.env["channel.pms.room.type"].search(
