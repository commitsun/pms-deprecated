...
class PmsFolio(models.Model):
    ...
    customer_notes = fields.Text(
        "Channel Customer Notes", readonly=True, old_name="wcustomer_notes"
    )
    ...
    def action_confirm(self):
        for rec in self:
            rec.reservation_ids.write({"to_assign": False})
    ...
    def get_grouped_reservations_json(self, state, import_all=False):
        ...
        for rline in self.reservation_ids:
            if (
                ...
                and (
                    (rline.state == "cancelled" and not rline.channel_modified)
                    or rline.state != "cancelled"
                )
            ):
                ...
                vals = {
                    "num": len(
                        self.reservation_ids.filtered(
                            ...
                            and (
                                (r.state == "cancelled" and not r.channel_modified)
    ...
    def _compute_has_cancelled_reservations_to_send(self):
        ...
        channel_pms_reserv_obj = self.env["channel.pms.reservation"]
        for record in self:
            splitted_reservation_ids = record.reservation_ids.filtered(
                lambda x: x.splitted
            )
            has_to_send = False
            for rline in splitted_reservation_ids:
                ...
                has_to_send = (
                    channel_pms_reserv_obj.search_count(
                        [
                            ("splitted", "=", True),
                            ("folio_id", "=", self.id),
                            ("to_send", "=", True),
