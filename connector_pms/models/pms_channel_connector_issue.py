...
class PmsChannelConnectorIssue(models.Model):
    _name = "pms.channel.connector.issue"
    _old_name = "wubook.issue"

    backend_id = fields.Many2one(
        "channel.backend", "Backend", required=True, ondelete="cascade", index=True
    )
    ...
    def create(self, vals):
        issue_id = super(PmsChannelConnectorIssue, self).create(vals)
    ...
    def mark_as_read(self):
        ...
        for record in self:
            if record.section == "reservation" and record.channel_object_id:
                ...
                record.to_read = False
        if any(reserv_ids):
            with self.backend_id.work_on("channel.pms.reservation") as work:
