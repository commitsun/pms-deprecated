...
class HotelRoomType(models.Model):
    ...
    capacity = fields.Integer(
        "Capacity", compute="_compute_capacity", store=True)
    ...
    def open_channel_bind_ids(self):
        ...
        action = self.env.ref(
            'connector_pms.channel_hotel_room_type_action').read()[0]
        action['views'] = [
            (self.env.ref('connector_pms.channel_hotel_room_type_view_form').id, 'form')]
        action['target'] = 'new'
...
class BindingHotelRoomTypeListener(Component):
    ...
    def on_record_write(self, record, fields=None):
        fields_to_check = ('name', 'list_price',
                           'total_rooms_count', 'board_service_room_type_ids')
