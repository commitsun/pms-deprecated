    ...
    def write(self, vals):
        ...
        if vals.get("room_type_id"):
            ...
            for item in room_type_ids:
                if item["new_room_type_id"] != item["old_room_type_id"]:
                    ...
                    tz_hotel = (
                        self.env["ir.default"]
                        .sudo()
                        .get("res.config.settings", "tz_hotel")
                    )
                    _today = fields.Date.context_today(self.with_context(tz=tz_hotel))

                    # update old room type values
                    old_channel_room_type = self.env["channel.pms.room.type"].search(
                        [("odoo_id", "=", item["old_room_type_id"])]
                    )

                    old_room_type_total_rooms_count = (
                        old_channel_room_type.total_rooms_count
                    )

                    if (
                        old_channel_room_type.default_quota
                        > old_room_type_total_rooms_count
                    ):
                        old_channel_room_type.default_quota = (
                            old_room_type_total_rooms_count
                    ...
                    if (
                        old_channel_room_type.default_max_avail
                        > old_room_type_total_rooms_count
                    ):
                        old_channel_room_type.default_max_avail = (
                            old_room_type_total_rooms_count
                    ...
                    if (
                        ...
                        > old_channel_room_type.capacity
                    ):
                        old_channel_room_type.ota_capacity = (
                            old_channel_room_type.odoo_id.get_capacity()
                        )
                        issue_backend_id = old_channel_room_type.backend_id.id
                        issue_internal_message = (
                            "OTA capacity updated to %d for Room Type %s."
                            % (
                                old_channel_room_type.ota_capacity,
                        ...
                        self.env["pms.channel.connector.issue"].create(
                            {
                    ...
                    channel_availability = (
                        self.env["channel.pms.room.type.availability"].search(
                            [
                    ...
                    if channel_availability:
                        ...
                        dfrom = date_range[0]
                        dto = (date_range[-1] + timedelta(days=1)).strftime(
                    ...
                    new_channel_room_type = self.env["channel.pms.room.type"].search(
                        [("odoo_id", "=", item["new_room_type_id"])]
                    )

                    new_room_type_total_rooms_count = (
                        new_channel_room_type.total_rooms_count
                    )

                    if (
                        new_channel_room_type.default_quota
                        > new_room_type_total_rooms_count
                    ):
                        new_channel_room_type.default_quota = (
                            new_room_type_total_rooms_count
                    ...
                    if (
                        new_channel_room_type.default_max_avail
                        > new_room_type_total_rooms_count
                    ):
                        new_channel_room_type.default_max_avail = (
                            new_room_type_total_rooms_count
                    ...
                    if (
                        ...
                        > new_channel_room_type.capacity
                    ):
                        new_channel_room_type.ota_capacity = (
                            new_channel_room_type.odoo_id.get_capacity()
                        )
            ...
            channel_backend = (
                ...
                .search([("odoo_id", "=", vals.get("room_type_id"))])
                .backend_id
            )
