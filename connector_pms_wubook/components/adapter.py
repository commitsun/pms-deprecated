...
DEFAULT_WUBOOK_DATETIME_FORMAT = "{} {}".format(
    DEFAULT_WUBOOK_DATE_FORMAT, DEFAULT_WUBOOK_TIME_FORMAT,
)
...
class WuBookAdapter(AbstractComponent):
    ...
    def push_activation(self, base_url, security_token):
        ...
        if rcode_a != 0:
            raise ChannelConnectorError(
                _("Can't activate push reservations"), {"message": results_a}
            )
        ...
        if rcode_b != 0:
            raise ChannelConnectorError(
                _("Can't activate push rooms"), {"message": results_b}
            )
    ...
    def create_room(
        ...
        rcode, results = self._server.new_room(
            ...
            boards,
            int(rtype),
            # min_price, # Issue limit for min_price and max_price is they
            # have to be higher than 5
            # max_price,
        )
        if rcode != 0:
            raise ChannelConnectorError(
                _("Can't create room in WuBook"), {"message": results}
            )
        return results
    ...
    def modify_room(
        ...
        if rcode != 0:
            raise ChannelConnectorError(
                _("Can't modify room in WuBook"),
                {"message": results, "channel_id": channel_room_id},
            )
        return results

    def delete_room(self, channel_room_id):
        rcode, results = self._server.del_room(
            self._session_info[0], self._session_info[1], channel_room_id
        )
        if rcode != 0:
            raise ChannelConnectorError(
                _("Can't delete room in WuBook"),
                {"message": results, "channel_id": channel_room_id},
            )
        return results
    ...
    def fetch_rooms(self, channel_room_id=0):
        ...
        if rcode != 0:
            raise ChannelConnectorError(
                _("Can't fetch room values from WuBook"),
                {"message": results, "channel_id": channel_room_id},
            )
        return results
    ...
    def fetch_rooms_values(self, date_from, date_to, rooms=False):
        ...
        # 1.- The returned restrictions are the Standard Restrictions (
        # default restriction plan).
        # The prices are related to the WuBook Parity, that is, pid = 0 (
        # unless you modify it, linking the
        # WuBook Parity to a specific pricing plan).
        # 2.- You simply can't download room information for days in the past
        # or more than 2 years in the future.
        date_today = fields.Date.today()
        date_2_years = (date_today + timedelta(days=365) * 2).strftime(
        ...
        rcode, results = self._server.fetch_rooms_values(
            self._session_info[0],
            self._session_info[1],
            date_from.strftime(DEFAULT_WUBOOK_DATE_FORMAT),
            date_to.strftime(DEFAULT_WUBOOK_DATE_FORMAT),
            rooms,
        )
        if rcode != 0:
            raise ChannelConnectorError(
                _("Can't fetch rooms values from WuBook"), {"message": results}
            )
        return results

    def update_availability(self, rooms_avail):
        rcode, results = self._server.update_sparse_avail(
            self._session_info[0], self._session_info[1], rooms_avail
        )
        if rcode != 0:
            raise ChannelConnectorError(
                _("Can't update rooms availability in WuBook"), {"message": results}
            )
        return results
    ...
    def corporate_fetch(self):
        ...
        if rcode != 0:
            raise ChannelConnectorError(
                _("Can't call 'corporate_fetch' from WuBook"), {"message": results}
            )
        return results
    ...
    def create_reservation(
        ...
        customer = {
            ...
            "arrival_hour": checkin.strftime("%H:%M"),
            "notes": notes,
        }
        rcode, results = self._server.new_reservation(
            self._session_info[0],
            self._session_info[1],
            checkin.strftime(DEFAULT_WUBOOK_DATE_FORMAT),
            checkout.strftime(DEFAULT_WUBOOK_DATE_FORMAT),
        ...
        if rcode != 0:
            raise ChannelConnectorError(
                _("Can't create reservations in wubook"),
                {"message": results, "date_from": checkin, "date_to": checkout},
            )
        return results

    def cancel_reservation(self, channel_reservation_id, reason=""):
        rcode, results = self._server.cancel_reservation(
            self._session_info[0], self._session_info[1], channel_reservation_id, reason
        )
        if rcode != 0:
            raise ChannelConnectorError(
                _("Can't cancel reservation in WuBook"),
                {"message": results, "channel_reservation_id": channel_reservation_id},
            )
        return results

    def fetch_new_bookings(self):
        rcode, results = self._server.fetch_new_bookings(
            self._session_info[0], self._session_info[1], 1, 0
        )
        if rcode != 0:
            raise ChannelConnectorError(
                _("Can't process reservations from wubook"), {"message": results}
            )
        return results
    ...
    def fetch_bookings(self, dfrom, dto):
        rcode, results = self._server.fetch_bookings(
            self._session_info[0],
            self._session_info[1],
            dfrom.strftime(DEFAULT_WUBOOK_DATE_FORMAT),
            dto.strftime(DEFAULT_WUBOOK_DATE_FORMAT),
            0,
            # When oncreated is 0, the filter is applied against the arrival
            # date
            1,
        )
        if rcode != 0:
    ...
    def fetch_booking(self, channel_reservation_id):
        rcode, results = self._server.fetch_booking(
            self._session_info[0], self._session_info[1], channel_reservation_id, 1
        )
        if rcode != 0:
            raise ChannelConnectorError(
                _("Can't process reservations from wubook"), {"message": results}
    ...
    def mark_bookings(self, channel_reservation_ids):
        rcode, results = self._server.mark_bookings(
            self._session_info[0], self._session_info[1], channel_reservation_ids
        )
        if rcode != 0:
            raise ChannelConnectorError(
                _("Can't mark as readed a reservation in wubook"),
                {
                    "message": results,
    ...
    def create_plan(self, name, daily=1):
        rcode, results = self._server.add_pricing_plan(
            self._session_info[0], self._session_info[1], name, daily
        )
        if rcode != 0:
            raise ChannelConnectorError(
                _("Can't add pricing plan to wubook"), {"message": results}
            )
        return results

    def create_vplan(self, name, pid, dtype, value):
        rcode, results = self._server.add_vplan(
            self._session_info[0], self._session_info[1], name, pid, dtype, value,
        )
        if rcode != 0:
            raise ChannelConnectorError(
                _("Can't add virtual pricing plan to wubook"), {"message": results}
            )
        return results
    ...
    def modify_vplan(self, pid, dtype, value):
        rcode, results = self._server.mod_vplans(
            self._session_info[0],
            self._session_info[1],
            [{"pid": pid, "variation": value, "variation_type": dtype}],
        )
        if rcode != 0:
            raise ChannelConnectorError(
                _("Can't modify virtual pricing plan in wubook"), {"message": results}
            )
        return results
    ...
    def delete_plan(self, channel_plan_id):
        ...
        if rcode != 0:
            raise ChannelConnectorError(
                _("Can't delete pricing plan from wubook"),
                {"message": results, "channel_plan_id": channel_plan_id},
            )
        return results

    def update_plan_name(self, channel_plan_id, new_name):
        rcode, results = self._server.update_plan_name(
            self._session_info[0], self._session_info[1], channel_plan_id, new_name
        )
        if rcode != 0:
            raise ChannelConnectorError(
                _("Can't update pricing plan name in wubook"),
                {"message": results, "channel_plan_id": channel_plan_id},
            )
        return results
    ...
    def update_plan_prices(self, channel_plan_id, date_from, prices):
        ...
        if rcode != 0:
            raise ChannelConnectorError(
                _("Can't update pricing plan in wubook"),
                {
                    "message": results,
    ...
    def update_plan_periods(self, channel_plan_id, periods):
        rcode, results = self._server.update_plan_periods(
            self._session_info[0], self._session_info[1], channel_plan_id, periods
        )
        if rcode != 0:
            raise ChannelConnectorError(
                _("Can't update pricing plan period in wubook"),
                {"message": results, "channel_plan_id": channel_plan_id},
            )
        return results
    ...
    def get_pricing_plans(self):
        ...
        if rcode != 0:
            raise ChannelConnectorError(
                _("Can't get pricing plans from wubook"), {"message": results}
            )
        return results
    ...
    def rplan_rplans(self):
        ...
        if rcode != 0:
            raise ChannelConnectorError(
                _("Can't fetch restriction plans from wubook"), {"message": results}
            )
        return results
    ...
    def wired_rplan_get_rplan_values(
        ...
        # fetch_rooms_values returns a KV structure for each room and for
        # each day
            ...
            for room_type in results:
                ...
                for daily_restriction in restrictions:
                    daily_restriction.update(
                        {"date": date.strftime(DEFAULT_WUBOOK_DATE_FORMAT)}
                    )
                    date = date + timedelta(days=1)
            results = {"0": results}
        else:
            # WuBook Knowledge Base: restriction plan besides the wubook
            # restrictions
    ...
    def update_rplan_values(self, channel_restriction_plan_id, date_from, values):
        ...
        if rcode != 0:
            raise ChannelConnectorError(
                _("Can't update plan restrictions on wubook"),
                {
                    "message": results,
                    "channel_restriction_plan_id": channel_restriction_plan_id,
                    "date_from": date_from,
    ...
    def create_rplan(self, name, compact=False):
        rcode, results = self._server.rplan_add_rplan(
            self._session_info[0], self._session_info[1], name, compact and 1 or 0
        )
        if rcode != 0:
            raise ChannelConnectorError(
                _("Can't create plan restriction in wubook"), {"message": results}
            )
        return results
    ...
    def rename_rplan(self, channel_restriction_plan_id, new_name):
        ...
        if rcode != 0:
            raise ChannelConnectorError(
                _("Can't rename plan restriction in wubook"),
                {
                    "message": results,
                    "channel_restriction_plan_id": channel_restriction_plan_id,
                },
    ...
    def delete_rplan(self, channel_restriction_plan_id):
        rcode, results = self._server.rplan_del_rplan(
            self._session_info[0], self._session_info[1], channel_restriction_plan_id
        )
        if rcode != 0:
            raise ChannelConnectorError(
                _("Can't delete plan restriction on wubook"),
                {
                    "message": results,
                    "channel_restriction_plan_id": channel_restriction_plan_id,
                },
    ...
    def get_channels_info(self):
        ...
        if not any(results):
            raise ChannelConnectorError(
                _("Can't import channels info from wubook"), {"message": results}
            )
        return results
