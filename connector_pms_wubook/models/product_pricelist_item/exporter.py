...
class ProductPricelistItemExporter(Component):
    ...
    def push_pricelist(self):
        ...
        if any(channel_unpushed):
            date_start = channel_unpushed[0].date_start
            date_end = channel_unpushed[-1].date_start
            ...
            for pr in pricelist_ids:
                ...
                for pt_id in product_tmpl_ids:
                    ...
                    if channel_room_type:
                        prices[pr.external_id].update(
                            {channel_room_type.external_id: []}
                        )
                            ...
                            prod = channel_room_type.product_id.with_context(
                                {
                                    ...
                                    "date": (date_start + timedelta(days=i)).strftime(
                                        DEFAULT_SERVER_DATE_FORMAT
                                    ),
                                }
                            )
                            prices[pr.external_id][
                                channel_room_type.external_id
                            ].append(prod.price)
            ...
            try:
                for k_pk, v_pk in prices.items():
                    if any(v_pk):
                        self.backend_adapter.update_plan_prices(
                            k_pk, date_start.strftime(DEFAULT_SERVER_DATE_FORMAT), v_pk
                        )
                ...
                channel_unpushed.with_context({"connector_no_export": True}).write(
