...
class ProductPricelistItemImporter(Component):
    ...
    @api.model
    def _generate_pricelist_items(
        self, channel_plan_id, date_from, date_to, plan_prices
    ):
        _logger.info(
            "==[CHANNEL->ODOO]==== PRICELISTS [%d] (%s - %s) ==",
            int(channel_plan_id),
            date_from,
            date_to,
        )
        ...
        pricelist_item_mapper = self.component(
            usage="import.mapper", model_name="channel.product.pricelist.item"
        )
        if pricelist_bind:
    ...
    def import_all_pricelist_values(self, date_from, date_to, rooms=None):
        ...
        for external_id in external_ids:
            if external_id:
                self.import_pricelist_values(
                    external_id, date_from, date_to, rooms=rooms
                )
        return True
    ...
    def import_pricelist_values(self, external_id, date_from, date_to, rooms=None):
        try:
            results = self.backend_adapter.fetch_plan_prices(
                external_id, date_from, date_to, rooms
            )
