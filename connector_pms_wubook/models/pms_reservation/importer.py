...
class PmsReservationImporter(Component):
    _inherit = "channel.pms.reservation.importer"
    ...
    def _get_board_services(self, broom, book, room_type_bind, persons):
        ...
        if book["id_channel"] == 0:
            board_service = (
                room_type_bind.board_service_room_type_ids.filtered(
                    lambda r: r.channel_service
                    == book["boards"][room_type_bind.external_id]
                ).id
                or None
            )
        # Booking.com Reservation Engine
            ...
            # Board services can be included in the rate plan and detected by
            # the WuBook API
            if detected_board and detected_board != "nb":
                board_service = (
                    room_type_bind.board_service_room_type_ids.filtered(
                        lambda r: r.channel_service == detected_board
                    ).id
                    or None
                )
                # NOTE: Board services are managed by day and by person
                board_service_amount = (
                    board_service
                    and room_type_bind.board_service_room_type_ids.browse(
                        board_service
                    ).amount
                    * persons
                )
            # else:
            #     detected_addons = broom.get('ancillary', {}).get('addons')
            #     or []
            #     # Board services can be included as a list of addons where
            #     the price is broken down by day and service
            #     boards_map = {"Breakfast": "bb"}
            #     for addon in detected_addons:
            #         detected_board = addon.get('type') in boards_map and
            #         boards_map[addon.get('type')]
            #         board_service =
            #         room_type_bind.board_service_room_type_ids.filtered(
            #             lambda r: r.channel_service == detected_board).id
            #             or None
            #         # TODO: what to do if more than one addon is shipped in
            #          this array ¿?
    ...
    def _generate_booking_vals(
        ...
        # occupancy = next((item for item in book['rooms_occupancies'] if
        # item["id"] == broom['room_id']), False)
        # if occupancy:
        #     persons = occupancy['occupancy']
        # Dates
        real_checkin_str = real_checkin.strftime(DEFAULT_SERVER_DATETIME_FORMAT)
        real_checkout_str = real_checkout.strftime(DEFAULT_SERVER_DATETIME_FORMAT)
        ...
        if "ancillary" in broom:
            ...
            if (
                "tax_inclusive" in broom["ancillary"]
                and not broom["ancillary"]["tax_inclusive"]
            ):
        ...
        board_service, board_service_amount = self._get_board_services(
            broom, book, room_type_bind, persons
        )
        ...
        for brday in broom["roomdays"]:
            wndate = (
                datetime.strptime(brday["day"], DEFAULT_WUBOOK_DATE_FORMAT)
            ...
            # TODO: Review different pricelist in the different booked rooms
            #  (folio in Odoo)

        # Get the special offer rate id if any or the rate_id sent by Wubook
        # / OTAs included in the roomdays
        rate_id = book.get("ancillary", {}).get("wubook_special_rate_id", rate_id)

        parity_rate_id = (
            self.env["channel.backend"]
            .search([("id", "=", self.backend_record.id)])
            .wubook_parity_pricelist_id
        )
        rate_id = int(rate_id)
        # WuBook API rate_id ( booked pricing plan: -1 Unknown, 0 WuBook
        # Parity or WuBook id of the plan)
        if rate_id > 0:
            rate_id = (
                self.env["channel.product.pricelist"]
                .search(
                    [
    ...
    def _generate_partner_vals(self, book):
        country_id = self.env["res.country"].search(
            [("code", "=", str(book["customer_country"]))], limit=1
        )
        # lang = self.env['res.lang'].search([('code', '=',
        # book['customer_language_iso'])], limit=1)
        return {
            "name": "{}, {}".format(book["customer_surname"], book["customer_name"]),
    ...
    def _get_book_dates(self, book):
        default_arrival_hour = (
            self.env["ir.default"]
            .sudo()
            .get("res.config.settings", "default_arrival_hour")
        )
        default_departure_hour = (
            self.env["ir.default"]
            .sudo()
            .get("res.config.settings", "default_departure_hour")
        )

        checkin = "{} {}".format(book["date_arrival"], default_arrival_hour)
        checkin_utc_dt = datetime.strptime(
            checkin, DEFAULT_WUBOOK_DATETIME_FORMAT
        ).replace(tzinfo=tz.gettz("UTC"))

        checkout = "{} {}".format(book["date_departure"], default_departure_hour)
        checkout_utc_dt = datetime.strptime(
            checkout, DEFAULT_WUBOOK_DATETIME_FORMAT
        ).replace(tzinfo=tz.gettz("UTC"))
    ...
    def _force_update_availability_wubook(self, binding):
        ...
        channel_availability = self.env["channel.pms.room.type.availability"].search(
            [
    ...
    def _update_reservation_binding(self, binding, book):
        ...
        if binding.partner_id.unconfirmed:
            binding.partner_id.write(self._generate_partner_vals(book))
        if is_cancellation:
            binding.odoo_id.with_context(
                {"connector_no_export": True, "ota_limits": False}
            ).action_cancel()
            self._force_update_availability_wubook(binding)
        elif binding.state == "cancelled":
            binding.with_context({"connector_no_export": True}).write(
                {"discount": 0.0, "state": "confirm"}
            )
    ...
    def wubook_modification(self, reservations, book):
        ...
        for broom in book["booked_rooms"]:
            ...
            if reservations:
                # REVIEW: modified_reservations is possibly useless (
                # used_room nerver
                # it will match with rooms in reservations recordset paramenter
                modified_codes = " ".join(str(e) for e in book["modified_reservations"])
                modified_reservations = self.env["channel.pms.reservation"].search(
                ...
                reservation = reservations.filtered(
                    lambda res: res.room_type_id == room_type_bind.odoo_id
                    and res.checkin == checkin
                    and res.checkout == checkout
                    and res.room_id.id not in used_rooms
                )
            if reservation:
                ...
                # Review quota if new reservation is a modification not
                # recognized in this method
                # because quota is __always__ decreased when creating
                # reservation, even in the overlapped days
    ...
    def _generate_reservations(self, bookings):
        ...
        channel_reserv_obj = self.env["channel.pms.reservation"]
        ...
        for book in bookings:  # This create a new folio
            ...
            crcode = (
                ...
                if book["channel_reservation_code"]
                else "undefined"
            )
            ...
            if crcode in failed_reservations:
                self.create_issue(
                    section="reservation",
                    internal_message="Can't process a reservation that "
                    "previously failed!",
                    channel_object_id=book["reservation_code"],
                )
                continue
            ...
            # Wubook send first the reservation, and then the reservation (
            # other time) with pay (payment_gateway_fee=
            if (
                not book["modified_reservations"]
                and book["payment_gateway_fee"]
                and res_company_obj.wu_journal_id != 0
            ):
                origin_reservation = (
                    self.env["channel.pms.reservation"]
                    .search([("external_id", "=", book["reservation_code"])], limit=1)
                    .odoo_id
                )
            ...
            if len(book["modified_reservations"]) > 0:
                ...
                if book["was_modified"] and is_cancellation:
                    ...
                    reservations = self.env["channel.pms.reservation"].search(
                        [
                    ...
                    if reservations:
                        new_books, old_reservations = self.wubook_modification(
                            reservations, book
                        )
                    if old_reservations:
                        for res in old_reservations:
                            ...
                            self._force_update_availability_wubook(
                                res.channel_bind_ids[0]
                            )
                    if len(new_books) == 0:
                        processed_rids.append(rcode)
                        continue
            ...
            for broom in new_books:
                ...
                while dates_checkin[0]:
                    ...
                    #     bs = self.env[
                    #     'pms.board.service.room.type'].browse(vals[
                    #     'board_service_room_id'])
                    #     price_room_services_set = vals['price_unit'] + (
                    #     bs.amount * len(broom['roomdays']))
                    #     vals.update({'unconfirmed_channel_price': True})
                    #     # check if difference is owing to misconfigured
                    #     board services
                    #     if price_room_services_set != book['amount']:
                    #         internal_reason = 'Please, review the board
                    #         services included in the reservation.'
                    #         self.create_issue(
                    #             section='reservation',
                    #             internal_message="Invalid reservation total
                    #             price! %.2f (calculated) != %.2f (wubook)
                    #             %s" % (
                    ...
                    free_rooms = room_type_bind.odoo_id.check_availability_room_type(
                        vals["checkin"],
                        (vals["checkout"] - timedelta(days=1)).strftime(
                    ...
                    if any(free_rooms):
                        ...
                        if split_booking:
                            if not split_booking_parent:
                                ...
                                # sql_constraint 'unique(backend_id,
                                # external_id)'
                        ...
                        if date_diff <= 0:
                            ...
                            self.create_issue(
                                section="reservation",
                                internal_message="Reservation imported with "
                                "overbooking state",
            ...
            if not any(failed_reservations) and any(reservations):
                ...
                if folio_id:
                    folio_id.with_context({"connector_no_export": True}).write(vals)
                else:
                    vals.update(
                        {"partner_id": partner_id.id, "wseed": book["sessionSeed"]}
                    )
                    folio_id = pms_folio_obj.with_context(
                        {"connector_no_export": True}
                    ).create(vals)
                ...
                for rline in rlines:
                    ...
                    # Force to_assign = true (fix the to_assign splitted
                    # reservations)
                    ...
                    if rline.parent_reservation:
                        ...
                        backend_id = (
                            ...
                            .search([("odoo_id", "=", rline.room_type_id.id)])
                            .backend_id
                        )
                        self.env[
                            "channel.pms.room.type.availability"
                        ].sudo().refresh_availability(
