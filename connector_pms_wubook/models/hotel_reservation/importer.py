...
class HotelReservationImporter(Component):
        ...
        try:
            ...
            if any(results):
                ...
                if any(processed_rids):
                    self.backend_adapter.mark_bookings(
                        list(set(processed_rids)))
    ...
    def _get_board_services(self, broom, book, room_type_bind, persons):
        ...
        # Booking.com Reservation Engine
            ...
            detected_board = book.get('ancillary', {}).get(
                'Detected Board') or None
    ...
    def _generate_booking_vals(self, broom, crcode, rcode, room_type_bind,
        ...
        for brday in broom['roomdays']:
            ...
            if dates_checkin[0].date() <= wndate < dates_checkout[0].date():
                ...
                if not tax_inclusive:
                    price_subtotal = book['amount'] - \
                        broom['ancillary']['taxes']
                    day_tax_weigh = brday['price'] * 100 / price_subtotal
                    amount_day_tax = broom['ancillary']['taxes'] * \
                        day_tax_weigh / 100
        ...
        rate_id = book.get('ancillary', {}).get(
            'wubook_special_rate_id', rate_id)
        ...
        if book['modified_reservations']:
            modified_codes = ' '.join(str(e)
                                      for e in book['modified_reservations'])
        binding_vals = {
            'backend_id': self.backend_record.id,
            'external_id': rcode,
        ...
        for broom in book['booked_rooms']:
            ...
            if reservations:
                ...
                reservation = reservations.filtered(
                    ...
                    and res.checkin == checkin and res.checkout == checkout
    ...
    def _generate_reservations(self, bookings):
        ...
        for book in bookings:   # This create a new folio
            ...
            if not book['modified_reservations'] and book['payment_gateway_fee'] and res_company_obj.wu_journal_id != 0:
                origin_reservation = self.env['channel.hotel.reservation'].search(
                    [("external_id", "=", book["reservation_code"])], limit=1).odoo_id
