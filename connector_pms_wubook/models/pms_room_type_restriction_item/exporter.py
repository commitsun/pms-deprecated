...
class PmsRoomTypeRestrictionItemExporter(Component):
    _inherit = "channel.pms.room.type.restriction.item.exporter"

    @api.model
    def push_restriction(self):
        channel_pms_room_type_obj = self.env["channel.pms.room.type"]
        channel_room_type_rest_obj = self.env["channel.pms.room.type.restriction"]
        channel_rest_item_obj = self.env["channel.pms.room.type.restriction.item"]
        ...
        if any(unpushed):
            date_start = unpushed[0].date
            date_end = unpushed[-1].date
            ...
            for rp in channel_restr_plan_ids:
                ...
                for room_type in room_type_ids:
                    if any(room_type.channel_bind_ids):
                        room_type_bind = channel_pms_room_type_obj.search(
                            [
                        ...
                        restrictions[rp.external_id].update({room_type_external_id: []})
                            ...
                            if restr:
                                restrictions[rp.external_id][
                                ...
                                ].append(
                                    {
                                        ...
                                        "closed_arrival": restr.closed_arrival
                                        and 1
                                        or 0,
                                        "closed_departure": restr.closed_departure
                                        and 1
                                        or 0,
                                    }
                                )
            ...
            try:
                ...
                unpushed.with_context({"connector_no_export": True}).write(
                    {"channel_pushed": True, "sync_date": fields.Datetime.now()}
                )
        return True

    @api.model
    def close_online_sales(self):
        channel_rest_plan = self.env["channel.pms.room.type.restriction"].search(
            [("backend_id", "=", self.backend_record.id)]
        )
        channel_rest_item = self.env["channel.pms.room.type.restriction.item"]
        channel_room_types = self.env["channel.pms.room.type"].search(
            [("external_id", "!=", "")]
        )
        for channel_room_type in channel_room_types:
            ...
            if today_restrictions:
                ...
                self.env["pms.room.type.restriction.item"].with_context(
