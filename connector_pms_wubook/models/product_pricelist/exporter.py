...
class ProductPricelistExporter(Component):
    ...
    def update_plan_name(self, binding):
        try:
            ...
            return self.backend_adapter.update_plan_name(
                binding.external_id, binding.name
            )
    ...
    def create_vplan(self, binding):
        try:
            ...
            if item_ids.price_discount > 0:
                ...
                dtype = 1
            # NOTE: price_surcharge is greater than zero for increasing the
            # price
            ...
            external_id = self.backend_adapter.create_vplan(
                binding.name, base_pricelist.channel_bind_ids.external_id, dtype, value,
            )
        except ChannelConnectorError as err:
            self.create_issue(
                section="pricelist",
    ...
    def modify_vplan(self, binding):
        try:
            ...
            return self.backend_adapter.modify_vplan(binding.external_id, dtype, value)
        except ChannelConnectorError as err:
            self.create_issue(
                section="pricelist",
                internal_message=str(err),
