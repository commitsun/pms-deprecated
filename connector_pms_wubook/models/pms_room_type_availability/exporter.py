...
class PmsRoomTypeAvailabilityExporter(Component):
    _inherit = "channel.pms.room.type.availability.exporter"

    def push_availability(self):
        channel_pms_room_type_obj = self.env["channel.pms.room.type"]
        channel_room_type_avail_ids = self.env[
        ...
        for room_type in room_types:
            if any(room_type.channel_bind_ids):
                ...
                for channel_room_type_avail in channel_room_type_avails:
                    date_dt = channel_room_type_avail.date
                    days.append(
                        {
        ...
        if any(avails):
            try:
                # For functions updating room values (like availability,
                # prices, restrictions and so on),
                # for example update_avail(), there is a maximum number of
                # updatable days (for __each room__)
                ...
                ).write({"channel_pushed": True, "sync_date": fields.Datetime.now()})
        return True
