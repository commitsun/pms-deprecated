...
class TestWubook(TestHotelWubook):
    def test_simple_booking(self):
        ...
        date_diff = (
            date_utils.date_diff(checkin_utc_dt, checkout_utc_dt, hours=False) + 1
        )
        ...
        self.assertTrue(
            any(nreserv.reservation_line_ids), "Reservation lines snot found"
        )
    ...
    def test_splitted_booking(self):
        ...
        self.assertEqual(
            nreservs[0].reservation_line_ids[0].date,
            date_dt.strftime(DEFAULT_SERVER_DATE_FORMAT),
            "Invalid split",
        )
        date_dt = date_utils.get_datetime(nreservs[1].checkin)
        self.assertEqual(
            nreservs[1].reservation_line_ids[0].date,
            date_dt.strftime(DEFAULT_SERVER_DATE_FORMAT),
            "Invalid split",
        )
        self.assertEqual(nreservs[0].price_unit, 30.0, "Invalid split price")
        self.assertEqual(nreservs[1].price_unit, 37.0, "Invalid split price")
        self.assertEqual(
            nreservs[1].parent_reservation.id,
            nreservs[0].id,
    ...
    def test_invalid_booking(self):
        ...
        self.assertEqual(nreservs[0].state, "draft", "Overbooking don't handled")
        self.assertTrue(nreservs[1].overbooking, "Overbooking don't handled")
    ...
    def test_overbooking(self):
        ...
        self.assertFalse(nreservs[1].overbooking, "Overbooking don't handled")
        self.assertTrue(nreservs[2].overbooking, "Overbooking don't handled")
